...
/* Display an error or alert */
	function complain(message,type) {
		if (!$('#system_messages').html()) { 
			$('<section id="system_messages" class="container" style="display:none;"><a href="#hideMessages" class="dismiss">OK</a><ul></ul></section>').insertAfter('header nav');
		}
		if (!type) { type = 'error'; } 
		$('#system_messages').removeClass().addClass(type).addClass('container');
